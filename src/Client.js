...
class Client extends EventEmitter {
        ...
        try {
            const needAuthentication = await this.pupPage.evaluate(async () => {
                ...
                if (state === 'OPENING' || state === 'UNLAUNCHED' || state === 'PAIRING') {
                    ...
                    console.log("Waiting till state changes");
                    await new Promise(r => {
                        console.log("k")
            ...
            console.log("an error has occired here");
            console.log("mmmm is ", e);
            this.destroy();
            var self = this;
            setTimeout(function () {
                self.emit(Events.AUTH_FAILED, true);
            },3000);
            return;
        }
    ...
    async sendMessage(chatId, content, options = {}) {
        ...
        const newMessage = await this.pupPage.evaluate(async (chatId, message, options, sendSeen) => {
            ...
            let retries = 1;
            let maxRetries = 6;
            while (!window.Store && retries < maxRetries) {
                console.log(`Failed on retry  ${retries}`)
                await delayExecution(3000);
                retries++;
            }
            console.log(`Passed on retry ${retries}`);
